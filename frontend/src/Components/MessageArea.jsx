...
function MessageArea({rec}) {
  ...
  useEffect(() => {
    if (receiver) {
      console.log("updating listMessages")
  ...
  return (
    ...
    <div className='relative text-5xl text-fuchsia-700 flex flex-col justify-between h-full'>
      ...
      {Object.entries(listMessages).map(([key,item]) =>(
        <SingleMessage key={key} currUser={currUser} text={item.text} sender={item.sender} />
      ))}

      </div>
      <div className='flex flex-row items-center w-full '>
      <input type="file" className="file-input file-input-accent w-24" value={file} onChange={(e)=> setFile(e.target.files[0])} />
      <input type="text" className="input flex-grow " placeholder="Your Message" value={message} onChange={(e)=>setMessage(e.target.value)}/>
      <button className="btn btn-accent w-12" onClick={handleSubmit} >Submit</button>
